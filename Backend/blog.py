...
def generate_blog_with_gemini(context, topic,style,length):
    """
    Generates a professional blog using the Gemini API.
    ...
    try:
        ...
        if response and hasattr(response, "text"):
            ...
            return response.text  # Extract valid JSON
        else:
...
def generate_image_from_prompt(image_prompt, output_path):
    ...
    Args:
        image_prompt (str): The textual description of the image.
        output_path (str): File path to save the generated image.
        
    Returns:
        str: Path of the saved image or error message.
    """
    try:
...
def generate_images_from_blog_json(blog_json, output_dir="static/blogs"):
    """
    Generate and save images from image prompts in the blog JSON.

    Args:
        blog_json (dict): Parsed blog JSON containing image prompts.
    ...
    for idx, prompt in enumerate(image_prompts):
        # filename = os.path.join(output_dir, f"image{idx + 1}.png")
...
def clean_json_string(json_string):
    """
    Cleans and prepares a JSON string by:
    - Removing markdown-style code blocks (` ```json ` and ` ``` `)
    - Stripping unnecessary whitespace
    - Replacing problematic control characters
    """
    # Remove markdown code block markers
    cleaned_string = re.sub(r"```json|```", "", json_string).strip()

    # Remove problematic control characters (e.g., non-printable characters)
...
def save_json_to_file(json_string, filename="corrected_blog.json"):
    """
    Saves a cleaned JSON string to a file after validating its format.

    Args:
        json_string (str): The JSON string to be cleaned and saved.
...
def save_markdown_file(blog_data, image_paths, output_dir="static/blogs"):
    ...
    # Add the full content block as-is
    markdown_lines.append(content.strip() + "\n")
    # print("geted the markdownlines ")
    # Write to .md file
    md_path = Path(output_dir) / "blogs.md"
    with open(md_path, "w", encoding="utf-8") as f:
        f.write("\n".join(markdown_lines))
    # print("saved hte markdonw")
...
def read_markdown_content(markdown_path: str) -> str:
    try:
        with open(markdown_path, "r", encoding="utf-8") as f:
            # print("sucessufull read : ",f.read())
            return f.read()
    except Exception as e:
        raise RuntimeError(f"Failed to read Markdown file: {str(e)}")


# save_json_to_file(blog_response)
